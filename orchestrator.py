...
class Orchestrator:
    ...
    def _append_record(self, record: NormalizedRecord) -> None:
        """Append record to dedup store and assets file, with compression for large files."""
        self.dedup.add(record)
        
        # Compress if file > 10MB to prevent runaway growth
        if os.path.exists(self._assets_path) and os.path.getsize(self._assets_path) > 10 * 1024 * 1024:
            with open(self._assets_path, 'rb') as f_in:
                with gzip.open(self._assets_path + '.gz', 'wb') as f_out:
                    shutil.copyfileobj(f_in, f_out)
            os.remove(self._assets_path)
            self._assets_path = self._assets_path + '.gz'
